...
import {Dog}  from 'contracts/system/dog.sol';
...
import {End}  from 'contracts/system/end.sol';
...
contract EndTest is DSTest {
    ...
    function zar(address urn) internal view returns (uint) {
        return vat.zar(urn) / RAY;
    }
    ...
    function test_cage_collateralised() public {
        ...
        // ali's urn has 0 gem, 10 ink, 15 tab, 15 zar
    ...
    function test_cage_undercollateralised() public {
        ...
        // bob's urn has 0 gem, 1 ink, 3 tab, 3 zar
    ...
    function test_cage_snip() public {
        ...
        assertEq(end.tag("gold"), ray(0.2 ether)); // par / price = collateral per ZAR
        ...
        assertEq(vat.zar(address(vow)),                  0); // vat.suck() hasn't been called
        ...
        assertEq(vat.zar(address(vow)),              tab1); // From vat.suck()
    ...
    function test_cage_undercollateralised_surplus() public {
        ...
        ali.frob("gold", urn1, urn1, urn1, 10 ether, 15 ether);
        // ali's urn has 0 gem, 10 ink, 15 tab, 15 zar
        // alive gives one zar to the vow, creating surplus
    ...
    function test_cage_net_undercollateralised_multiple_ilks() public {
        ...
        gold.pip.poke(bytes32(2 * WAD));
        // urn1 has 20 zar of ink and 15 zar of tab
        coal.pip.poke(bytes32(2 * WAD));
        // urn2 has 2 zar of ink and 5 zar of tab
        end.cage();
        end.cage("gold");
        end.cage("coal");
        end.skim("gold", urn1);  // over-collateralised
        ...
        // the gold is worth 15 zar and the coal is worth 2 zar
        // the total collateral pool is worth 17 zar
        // the total outstanding debt is 20 zar
        // each zar should get (15/2)/20 gold and (2/2)/20 coal
    ...
    function test_overflow() public {
        ...
        // ali's urn has 500_000 ink, 10^6 art (and 10^6 zar since rate == RAY)
    ...
    uint256 constant REDEEM_AMT = 1_000 * WAD;  // Amount of ZAR to redeem for error checking
    function test_fuzz_fix_calcs_0_1(uint256 col_seed, uint192 debt_seed) public {
        ...
        // how much collateral can be obtained with a single ZAR in each case
