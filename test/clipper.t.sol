...
import {Spotter} from "contracts/system/spot.sol";
import {GemJoin} from "contracts/system/join.sol";
import {ZarJoin} from "contracts/system/join.sol";

import {Clipper} from "contracts/system/clip.sol";
import "contracts/system/abaci.sol";
import "contracts/system/dog.sol";
...
contract Exchange {
    ...
    constructor(MockToken gold_, MockToken zar_, uint256 goldPrice_) {
        gold = gold_;
        zar = zar_;
    ...
    function sellGold(uint256 goldAmt) external {
        ...
        uint256 zarAmt = (goldAmt * goldPrice) / 1e18;
        zar.transfer(msg.sender, zarAmt);
    }
}
...
contract Trader {
    ...
    function clipperCall(address sender, uint256 owe, uint256 slice, bytes calldata data) external {
        ...
        zar.approve(address(zarJoin));
        vat.hope(address(clip));
        zarJoin.join(sender, owe / 1e27);
    }
}
...
contract ClipperTest is DSTest {
    ...
    modifier takeSetup() {
        ...
        assertEq(vat.zar(ali), rad(1000 ether));
        assertEq(vat.gem(ilk, bob), 0);
        assertEq(vat.zar(bob), rad(1000 ether));
    ...
    function setUp() public {
        ...
        zar = new MockToken("Zar");
        zarJoin = new ZarJoin(address(vat), address(zar));
        vat.suck(address(0), address(zarJoin), rad(1000 ether));
        exchange = new Exchange(gold, zar, (goldPrice * 11) / 10);

        zar.mint(1000 ether);
        zar.transfer(address(exchange), 1000 ether);
        ...
        che = address(new Trader(clip, vat, gold, goldJoin, zar, zarJoin, exchange));
    ...
    function test_kick() public {
        ...
        clip.file("tip", rad(100 ether)); // Flat fee of 100 Zar
        ...
        assertEq(vat.gem(ilk, me), 960 ether);
        assertEq(vat.zar(ali), rad(1100 ether)); // Paid "tip" amount of Zar for calling bark()
        (ink, art) = vat.urns(ilk, me);
        ...
        assertEq(vat.zar(bob), rad(1000 ether) + rad(100 ether) + (tab * 0.02 ether) / WAD); // Paid (tip + due * chip) amount of zar for calling bark()
    }
    ...
    function test_bark_not_leaving_dust_rate() public {
        ...
        dog.file(ilk, "chop", 1 ether); // 0% chop for precise calculations
        vat.file(ilk, "dust", 20 * RAD); // 20 zar minimum Vault debt
        clip.upchost();
        ...
        (ink, art) = vat.urns(ilk, me);
        assertEq(ink, 40 ether);
        assertEq(art, 100 ether); // Full debt is 102 zar since rate = 1.02 * RAY
    ...
    function test_bark_only_leaving_dust_over_hole_rate() public {
        ...
        dog.file(ilk, "chop", 1 ether); // 0% chop for precise calculations
        vat.file(ilk, "dust", (204 * RAD) / 10); // 20.4 zar dust
        clip.upchost();
    ...
    function test_take_over_tab() public takeSetup {
        ...
        assertEq(vat.zar(ali), rad(890 ether)); // Didn't pay more than tab (110)
    ...
    function test_take_at_tab() public takeSetup {
        ...
        assertEq(vat.gem(ilk, ali), 22 ether); // Didn't take whole lot
        assertEq(vat.zar(ali), rad(890 ether)); // Paid full tab (110)
        assertEq(vat.gem(ilk, me), 978 ether); // 960 + (40 - 22) returned to usr
    ...
    function test_take_under_tab() public takeSetup {
        ...
        assertEq(vat.zar(ali), rad(945 ether)); // Paid half tab (55)
    ...
    function test_take_full_lot_partial_tab() public takeSetup {
        ...
        assertTrue(sub(vat.zar(ali), rad(900 ether)) < rad(0.1 ether)); // Paid about 100 ether
    ...
    function test_take_bid_recalculates_due_to_chost_check() public takeSetup {
        ...
        Guy(ali).take({
            id: 1,
            amt: 18 * WAD, // Costs 90 zar at current price; 110 - 90 == 20 < 22 == chost
            max: ray(5 ether),
            who: address(ali),
            data: ""
        });
    ...
    function test_take_multiple_bids_different_prices() public takeSetup {
        ...
        assertEq(vat.zar(ali), rad(950 ether)); // Paid some tab (50)
        assertEq(vat.gem(ilk, me), 960 ether); // Collateral not returned (yet)
        ...
        assertEq(vat.zar(bob), rad(940 ether)); // Paid rest of tab (60)
    ...
    function test_redo_incentive() public takeSetup {
        ...
        assertEq(vat.zar(address(123)), clip.tip());

        clip.file("chip", 0.02 ether); // Reward 2% of tab
        hevm.warp(block.timestamp + 300);
        clip.redo(1, address(234));
        assertEq(vat.zar(address(234)), clip.tip() + (clip.chip() * tab) / WAD);

        clip.file("tip", 0); // No more flat fee
        hevm.warp(block.timestamp + 300);
        clip.redo(1, address(345));
        assertEq(vat.zar(address(345)), (clip.chip() * tab) / WAD);

        vat.file(ilk, "dust", rad(100 ether) + 1); // ensure wmul(dust, chop) > 110 zar (tab)
        clip.upchost();
        assertEq(clip.chost(), 110 * RAD + 1);

        hevm.warp(block.timestamp + 300);
        clip.redo(1, address(456));
        assertEq(vat.zar(address(456)), 0);
        ...
        assertEq(tab, rad(110 ether) - 38 ether * price); // > 22 zar chost
        ...
        assertEq(vat.zar(address(567)), 0);
    }
    ...
    function test_remove_id() public {
        PublicClip pclip = new PublicClip(
            address(vat),
            address(spot),
            address(dog),
            "gold"
